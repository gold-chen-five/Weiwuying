...
UI.Web_info_cancel = $('.web_info_cancel')
...
UI.Web_info_change_by_language = (language) => {
    const web_info_text = [
            ...
            'For best performance, it is recommended to use the up-to-date version of Chrome or FireFox browser, and set resolution higher than 1024x768. During using a mobile phone browsing, please run on iOS 14 or Android 12 or above.',
            '当ウェブサイトをより快適にご利用いただくため、 Chrome や Firefoxのブラウザで、 解像度1024x76の利用をお願いします。スマホの方は、お使いのブラウザはiOS 14やAndroid 12以上のバージョンをご利用お願いします。'
    ]
    if (language === '.chinese') {
...
UI.Web_info_cancel.on('click', () => {
    UI.Web_info.css('display', 'none')
})
