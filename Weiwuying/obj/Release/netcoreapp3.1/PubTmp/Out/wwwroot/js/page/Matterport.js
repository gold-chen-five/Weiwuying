...
MatterportPage.Tour_end = $('.tour_end')
//導覽選項
...
MatterportPage.Tour_5_p = $('.tour_5_p')
/*---------------------------------------------------------
 [初始變數]*/
const tour_text = {
    A: {
        chinese: ['歌劇院', '音樂廳', '戲劇院', '表演廳', '樹冠大廳入口','入口大廳入口'],
        english: ['Opera Hose', 'Concert Hall', 'Playhouse', 'Recital House', 'Crown Hall Entrance','Main Entrance'],
        japanese: ['オペラハウス', 'コンサートホール', 'プレイハウス', 'リサイタルホール', '樹冠ロビー入り口','エントランスホール入り口']
    },
    B: {
        chinese: ['歌劇院', '音樂廳', '戲劇院', '表演廳', '樹冠大廳入口', '入口大廳入口'],
        english: ['Opera Hose', 'Concert Hall', 'Playhouse', 'Recital House', 'Crown Hall Entrance', 'Main Entrance'],
...
MatterportPage.Tour_end.on('click', () => MatterportPage.Left_Menu_Cancel_Onclick())
/*-------------------------------------------------------
...
MatterportPage.Roomour_execute = () => {
    if (MatterportPage.Left_Menu_Player) {
        MatterportPage.Tour.trigger('play')
        MatterportPage.Tour.on('ended', () => {
            MatterportPage.Player_css()
        })
    }
    else {
        MatterportPage.Tour.trigger('pause')
    }
}
MatterportPage.Player_Roomtour = () => {
    //MatterportPage.Tour.css('display', 'block')
...
MatterportPage.Line_A.on('click', () => {
    ...
    MatterportPage.Tour.attr('src', `../../models/Video/video_tour2/${MatterportPage.Left_Menu_Line_Selected}_${MatterportPage.Left_Menu_Selected}.mp4`)
    //MatterportPage.Left_Menu_Player ? MatterportPage.Left_Menu_Player_Onclick() : MatterportPage.Player_Roomtour()
    if (MatterportPage.Left_Menu_Player) MatterportPage.Player_css()
})
...
MatterportPage.Tour_5.on('click', () => MatterportPage.Left_Menu_Onclick(5))
